...
function summarizeRoles(turns: any[]): string[] {
  ...
  const assistantKeywords = extractKeywords(
    turns.filter((turn) => turn.role === "assistant").map((turn) => turn.content),
    12
  );
...
function buildAutoSummary(sessionId: string): { projectId: string; summary: string } | null {
...
function saveAutoSummary(sessionId: string, reason: string): boolean {
