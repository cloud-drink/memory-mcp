...
app.get('/api/activities', (req, res) => {
    const activities = db.db.prepare('SELECT * FROM activity_logs ORDER BY timestamp DESC LIMIT 500').all();
    res.json(activities);
});
// API: Delete memory
...
app.get('/api/turns', (req, res) => {
    const turns = db.db.prepare('SELECT * FROM raw_turns ORDER BY timestamp DESC LIMIT 1000').all();
    res.json(turns);
});
